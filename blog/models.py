...
class Carousel(models.Model):
    ...
    img = models.FileField(upload_to='carousel',verbose_name=u'轮播图片')
    # article = models.ForeignKey(Article,null=True,verbose_name=u'文章')
    create_time = models.DateTimeField(u'创建时间',auto_now_add=True)
    class Meta:
...
class FrontPic(models.Model):
    title = models.CharField(max_length=100,verbose_name=u'标题')
    img = models.FileField(upload_to='frontpic',verbose_name=u'首页图片')
    create_time = models.DateTimeField(u'创建时间',auto_now_add=True)
    class Meta:
...
class ClubIntro(models.Model):
    title = models.CharField(max_length=100,verbose_name=u'标题')
    img = models.FileField(upload_to='Clubintropic',verbose_name=u'Club介绍图片')
    create_time = models.DateTimeField(u'创建时间',auto_now_add=True)
    class Meta:
        verbose_name_plural = verbose_name = u'club介绍图片'
        ordering = ['-create_time']
        app_label = string_with_title('blog',u"博客管理")
